...
Uses match_dyntaxa_taxa() to find and match taxa names provided in the data against the Dyntaxa database. Please note that you need to have a subscription key to use the Dyntaxa API. You can get a key by registering at https://api-portal.artdatabanken.se/. For more information on how to set the API key see `help(is_in_dyntaxa)`.

In addition to the Dyntaxa match it is recommended to run the match_taxa() function to match against the WoRMS database.
