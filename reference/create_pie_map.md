...
- group_labels:
  ...
  Labels may include HTML markup (requires the `ggtext` package to
  render).
...
- size_by:
  ...
  numeric per-station column in `data` (e.g. chlorophyll, secchi depth);
  values are averaged per station when duplicated across rows.
