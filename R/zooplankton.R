...
calc_zooplankton_dry_weight <- function(data,
  ...
  if (nrow(general_nauplii) == 0) {
    cli::cli_abort("General copepod nauplii coefficients could not be loaded.")
  }
  if (nrow(general_nauplii) > 1) {
    cli::cli_warn("Multiple rows for generic copepod nauplii (AphiaID {GENERIC_COPEPOD_NAUPLII_APHIA_ID}) in coefficient workbook; using the first.")
  ...
  if (n_invalid_length > 0) {
    cli::cli_warn("{n_invalid_length} {length_parameter} row{?s} with non-positive value were dropped from dry-weight calculation.")
  }
...
load_zooplankton_dry_weight_coefficients <- function() {
  ...
  if (!nzchar(coeff_path)) {
    cli::cli_abort("Could not locate zooplankton dry-weight coefficient workbook.")
  }
