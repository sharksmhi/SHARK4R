...
SHARK sources taxonomic information from [Dyntaxa](https://sharksmhi.github.io/SHARK4R/articles/retrieve_dyntaxa_data.html), which is reflected in columns starting with `taxon_xxxxx`. Equivalent columns based on WoRMS can be retrieved using the [`update_worms_taxonomy`](../reference/update_worms_taxonomy.html) function.
...
major_stations <- shark_data %>%
  filter(station_name %in% c("BY31 LANDSORTSDJ",
                             "ANHOLT E",
                             "NB1 / B3"))

# Randomly select one sample from the major_stations
sample <- sample(major_stations$shark_sample_id_md5, 1)
...
plankton_groups <- assign_phytoplankton_group(shark_data_subset$scientific_name,
                                              shark_data_subset$aphia_id,
                                              verbose = FALSE)

# Print result
tibble(distinct(plankton_groups))

# Summarize abundance results based on plankton groups
...
# Plot a pie chart
ggplot(plankton_group_sum, aes(x = "", y = sum_plankton_groups, fill = plankton_group)) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  labs(
...
You can add custom plankton groups by using the `custom_groups` parameter, allowing flexibility to categorize plankton based on specific taxonomic criteria.
...
# Assign groups by providing scientific name only, and adding two custom groups
plankton_groups <- assign_phytoplankton_group(shark_data_subset$scientific_name,
                                              custom_groups = custom_groups,
                                              verbose = FALSE)

# Summarize abundance results based on new plankton groups
...
ggplot(plankton_custom_group_sum, aes(x = "", y = sum_plankton_groups, fill = plankton_group)) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  labs(
