...
Taxonomic records can be retrieved for individual genera names using the `get_algaebase_genus()` function.
...
Taxonomic records can be retrieved for individual species names using the `get_algaebase_species()` function.
...
Multiple names can be matched with the `match_algaebase()` function. The scientific names need to be parsed into `genus` and `species` names before being passed to the API, which can be achieved by the `parse_scientific_names()` function.
