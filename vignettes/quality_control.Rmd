...
```{r}
n_rows_on_land <- tryCatch(
  check_onland(chlorophyll_data),
  error = function(e) {
    message("OBIS xylookup API unavailable: ", conditionMessage(e))
    NULL
  }
)
if (!is.null(n_rows_on_land)) nrow(n_rows_on_land)
```
...
tryCatch(
  ...
  error = function(e) message("OBIS xylookup API unavailable: ", conditionMessage(e))
) # default columns: min/max depth
tryCatch(
